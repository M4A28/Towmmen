...
@Composable
fun EditInvoiceScreen(navController: NavHostController){
    ...
    navController.previousBackStackEntry
        ?.savedStateHandle
        ?.get<String?>(INVOICE_ID)?.let {
            val invoiceItem by invoiceViewModel.getInvoiceItems(it).collectAsState(initial = listOf())
            val invoice by invoiceViewModel.getInvoiceById(it).collectAsState(initial = Invoice(
                invoiceId = "",
                customerId = 0,
                customerName = "",
                date = Date(),
            ...
            EditInvoiceContent(
                ...
                onEditInvoiceClick = { oldInvoice, oldItems, newInvoice, newItems ->

                    oldItems.forEach { item ->
                            val product = products.firstOrNull { it.productId == item.productId }
                            if(product != null){
                    ...
                    invoiceViewModel.deleteInvoice(oldInvoice)
                    invoiceViewModel.insertFullInvoice(newInvoice, newItems)
...
fun EditInvoiceContent(
    ...
    onEditInvoiceClick: (Invoice, List<InvoiceItem>, Invoice, List<InvoiceItem>) -> Unit
) {
    var barcodeValue by remember { mutableStateOf("") }
    var searchQuery by remember { mutableStateOf(barcodeValue.ifEmpty { "" }) }
    val invoiceItem by remember { mutableStateOf(invoiceItems.toMutableList()) }
    /*invoiceItem.addAll(invoiceItems)*/
    var finalInvoice by remember{ mutableStateOf<Invoice?>(invoice) }
        ...
        topBar = {
            TopAppBar(
                title = {
                    ...
                    ) { Text(text = stringResource(id = R.string.edit_invoice)) }
                }
            )
        }
    ){ paddingValue ->
                ...
                onConform = {
                    try{
                        onEditInvoiceClick(
                            ...
                            invoice, invoiceItem,
                             // new Items
                            finalInvoice!!.copy(customerName = it), invoiceItem.toMutableList())
